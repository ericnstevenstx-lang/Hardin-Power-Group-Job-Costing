...
export default function DrawingIntakePage() {
  ...
  function printBom() {
    if (!jobCustomer.trim()) return setError('Enter a customer name before printing.');
    if (!poNumber.trim()) return setError('Enter a PO or Estimate number before printing.');
    setError('');
    window.print();
  }
    ...
    <>
      ...
      <style>{`
        @media print {
          ...
          #print-bom, #print-bom * { visibility: visible !important; }
          #print-bom { position: fixed; top: 0; left: 0; width: 100%; padding: 24px 32px; background: #fff; color: #000; font-family: 'IBM Plex Mono', monospace; }
          #print-bom h2 { font-size: 14px; font-weight: 600; margin: 0 0 2px; }
          #print-bom .ph { font-size: 10px; color: #555; margin: 0 0 12px; }
          #print-bom table { width: 100%; border-collapse: collapse; font-size: 10px; margin-top: 10px; }
          #print-bom th { text-align: left; border-bottom: 1px solid #000; padding: 4px 6px 4px 0; font-weight: 600; }
          #print-bom td { padding: 3px 6px 3px 0; border-bottom: 1px solid #ddd; }
          #print-bom td:last-child, #print-bom th:last-child { text-align: right; }
          #print-bom .total { font-weight: 600; font-size: 11px; margin-top: 8px; text-align: right; }
          #print-bom .footer { font-size: 9px; color: #777; margin-top: 16px; border-top: 1px solid #ccc; padding-top: 6px; }
        }
        @media screen { #print-bom { display: none; } }
      `}</style>
        ...
        <>
          ...
          <div className="card" style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginBottom: 12 }}>
              ...
              Pick from the COGS catalog to see historical purchase prices, or use the free-text option for one-off items.
            </div>

            <div style={{ marginBottom: 16 }}>
              <ComponentPicker
                onAdd={(c) => setComponents(prev => [...prev, {
                  description: c.description,
                  qty: c.qty,
                  unit_cost: c.unit_cost,
                  part_catalog_id: c.part_catalog_id || null,
            ...
            {presets.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
                <span style={{ fontSize: 11, color: 'var(--color-text-tertiary)', alignSelf: 'center' }}>Quick add (legacy):</span>
                {presets.map(p => (
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={printBom} style={{ marginLeft: 8 }}>⎙ Print BOM</button>
